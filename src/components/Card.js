...
const Card = ({ ele, pLiked, setprogress }) => {
  ...
  if (post === 1) alert("danger", "User not found");
  const getPost = async () => {
    try {
      let res = await axios.get(`https://dp1.sytes.net/posts/${id}`);
      let res1 = await axios.get(`https://dp1.sytes.net/likedid`);

      if (res1.status == 200) setlikedId(res1.data.id);
      else throw new Error(res1.data);
  ...
  if (copied) alert("success", "link copied to clipboard");
  ...
  if (postLikes == 1) likes = "1 Like";
  else if (postLikes > 1) likes = `${postLikes} Likes`;
  else likes = "";
  ...
  socket.on("getlikes", (id, likes) => {
    if (post.id === id) setpostLikes(likes);
  });
  ...
  const likeUnlike = async () => {
    try {
      ...
      if (like == 0) {
        ...
        let res = await axios.post("https://dp1.sytes.net/like", {
          id: post.id,
        });
        ...
        let res1 = await axios.post("https://dp1.sytes.net/unlike", {
          id: post.id,
        });
        if (res1.status != 200) throw new Error(res1.data);
        else {
          unfill();
          setLike(0);
          setprogress(100);
    ...
    <div className={cardContainer}>
      {post != 1 && post != null ? (
        <div className="card">
          ...
          >
            <img
              src={`https://dp1.sytes.net/getimg/${post.id}`}
              alt="img"
              className="img"
            />
          </div>
          <div className="card-bottom">
            <span id="heart">
              <i
                className={`fa ${fillColor} heart`}
                aria-hidden="true"
                onClick={(e) => likeUnlike()}
              ></i>{" "}
            </span>
            <CopyToClipboard
              text={`https://dp.turntbloke.tech/post/${post.id}`}
              onCopy={() => setCopied(true)}
            >
              <span id="share">
                <svg
                  aria-label="Share Post"
                  className="_8-yf5 fa-heart-o share"
                  color="#262626"
                  fill="#262626"
                  height="24"
                  role="img"
